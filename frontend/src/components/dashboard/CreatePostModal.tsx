...
export default function CreatePostModal({
    ...
    const handlePost = async () => {
        ...
        const token = authToken || (typeof window !== "undefined" ? (localStorage.getItem('baap_token') || localStorage.getItem('token')) : null);
        if (!token) {
            setToast({ message: "Authentication session expired. Please log in again.", type: 'error' });
            return;
        }
