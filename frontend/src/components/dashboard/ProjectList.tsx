    ...
    return (
        <div className="animate-in fade-in duration-500 pb-10">
            ...
            <div className="bg-white rounded-2xl border border-[#E2E8F0] shadow-sm mb-10 overflow-hidden">
                <div className="px-4 sm:px-8 py-4 sm:py-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-zinc-50/30">
                    ...
                    <button
                        ...
                        className="px-5 min-h-[44px] flex items-center justify-center bg-[#524EEE] text-white rounded-xl text-sm font-bold shadow-lg shadow-indigo-100 hover:scale-105 transition-transform active:scale-95 w-full sm:w-auto"
                    >
                        + New Project
                    </button>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 max-w-full lg:max-w-6xl w-full mx-auto">
                {displayPosts.length > 0 ? displayPosts.map((post: any) => (
                    <div key={post.id} className="bg-white rounded-2xl border border-gray-100 p-4 sm:p-5 md:p-[25px] shadow-sm hover:shadow-md hover:border-indigo-100 transition-all cursor-pointer group flex flex-col h-full">
                        <div className="flex items-start md:items-center justify-between gap-3 mb-3 flex-col md:flex-row">
                            <h3 className="font-black text-xl sm:text-2xl text-[#111827] tracking-tight group-hover:text-[#524EEE] transition-colors">{post.title}</h3>
                            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest bg-gray-50 px-2 py-1 rounded-md">
                                    {(() => {
                                        ...
                                        try {
                                            const utcStr = post.created_at.includes('T') && !post.created_at.endsWith('Z') && !post.created_at.includes('+') 
                                                ? `${post.created_at}Z` 
                                                : post.created_at;
                        ...
                        <div className="flex items-center justify-between pt-5 border-t border-gray-50 mt-auto flex-wrap gap-4">
                            <div className="flex items-center gap-3 w-full sm:w-auto">
                                <Link href={`/profile/${post.author_id}`} className="w-11 h-11 shrink-0 rounded-full bg-indigo-50 flex items-center justify-center text-indigo-500 shadow-inner overflow-hidden border border-white cursor-pointer hover:ring-2 hover:ring-indigo-400/50 transition-all">
                                    {post.profile_pic_url ? (
                                        <img src={post.profile_pic_url} alt="" className="w-full h-full object-cover" />
                                    ) : (
                                        <span className="font-bold text-sm">{(post.author_name || "?").charAt(0)}</span>
                                    )}
                                </Link>
                                <Link href={`/profile/${post.author_id}`} className="text-sm font-bold text-gray-700 cursor-pointer hover:text-[#524EEE] transition-colors py-2">{post.author_name || "Baap User"}</Link>
                            </div>
                            ...
                            {String(loggedInUser?.id) === String(post.author_id) ? (
                                <div className="flex items-center gap-2">
                                    <button
                                        ...
                                        className="px-4 min-h-[44px] flex items-center justify-center bg-indigo-50 text-[#524EEE] font-bold rounded-xl text-xs hover:bg-[#524EEE] hover:text-white transition-all shadow-sm"
                                    >
                                        Check Applicants
                                    </button>
                                    <button
                                        onClick={(e) => { e.stopPropagation(); setDeletingPostId(post.id); }}
                                        className="px-3.5 min-h-[44px] flex items-center justify-center bg-red-50 text-red-500 font-bold rounded-xl hover:bg-red-500 hover:text-white transition-all shadow-sm"
                                ...
                                <button
                                    ...
                                    className="px-5 min-h-[44px] flex items-center justify-center bg-white text-gray-500 font-bold rounded-xl text-xs border border-gray-200 shadow-sm hover:border-red-200 hover:text-red-500 hover:bg-red-50 transition-all disabled:opacity-50"
                                >
                                ...
                                <button
                                    onClick={(e) => handleApply(e, post.id)}
                                    disabled={applyingId === post.id}
                                    className="px-5 min-h-[44px] flex items-center justify-center bg-gray-50 text-gray-900 font-bold rounded-xl text-xs hover:bg-[#524EEE] hover:text-white transition-all shadow-sm disabled:opacity-50"
                                >
            ...
            {deletingPostId && (
                <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
                    ...
                    <div className="bg-white rounded-3xl w-full max-w-[calc(100%-2rem)] sm:max-w-sm mx-4 p-6 relative z-10 shadow-2xl animate-in fade-in zoom-in-95 duration-200 text-center">
            ...
            {viewingApplicantsId && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
                    ...
                    <div className="bg-white rounded-3xl w-full max-w-[calc(100%-2rem)] sm:max-w-lg p-5 sm:p-8 mx-4 relative z-10 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
                        <div className="flex items-center justify-between mb-6">
                            <h3 className="text-xl font-black text-gray-900">Project Applicants</h3>
                            <button onClick={() => setViewingApplicantsId(null)} className="w-11 h-11 bg-gray-100 rounded-full text-gray-500 hover:text-gray-900 hover:bg-gray-200 transition-colors flex items-center justify-center shrink-0">
                        ...
                        {loadingApplicants ? (
                            ...
                            <div className="flex flex-col gap-3 max-h-[60vh] overflow-y-auto pr-2">
                                {applicantsData.map(app => (
                                    <Link key={app.id} href={`/profile/${app.id}`} className="flex items-center gap-3 sm:gap-4 p-3 sm:p-4 rounded-2xl border border-gray-100 bg-white hover:border-indigo-200 hover:shadow-md transition-all group">
